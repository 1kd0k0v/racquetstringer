...
object RacquetTensionUtils {
    ...
    fun calculateStringTension(hz: Float, context: Context): Double {
        ...
        val stringDensity =
            StringDataArrayUtils.getStringDensity(SharedPrefsUtils.getStringType(context))
        val stringerStyle =
            StringDataArrayUtils.getStringerStyle(SharedPrefsUtils.getStringersStyle(context))
        val frame = StringDataArrayUtils.getStringOpeningSize(SharedPrefsUtils.getFrame(context))
        val stringPattern =
            StringDataArrayUtils.getStringPattern(SharedPrefsUtils.getStringPattern(context))
        ...
        return if (SharedPrefsUtils.isTensoinImperialUnits(context)) {
            if (SharedPrefsUtils.isCalibrated(context) && tension != 0.0) {
                NumberFormatUtils.format(
                    UnitUtils.kiloToPound(tension)
                        .toFloat() + SharedPrefsUtils.getTensionAdjustment(context)
                )
            } else {
